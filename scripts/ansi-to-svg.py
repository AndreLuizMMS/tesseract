#!/usr/bin/env python3
"""Converts Tesseract's ANSI output into SVG, for the repository's showcase.

Each character gets its own position on the x axis, so the grid stays
aligned even if the reader's machine doesn't have the stack's monospace
font. Contiguous backgrounds become a single rectangle.
"""
import re
import sys
import html
...
# Box-drawing characters rendered as geometry, not as text: the reader's font
# doesn't close the corners, and the grid would come out dashed. Each entry
# says which arms come out of the cell's center and at what weight (1 thin,
# 2 thick).
BRACOS = {
    "─": ("lr", 1), "│": ("ud", 1), "┌": ("rd", 1), "┐": ("ld", 1),
...
def tracos(linhas):
    """Returns the rectangles that form the grid, grouping adjacent ones."""
    saida = []
...
def cor(codigo, i):
    """Reads a color starting at SGR parameter i. Returns (color, next i)."""
...
def celulas(texto):
    """Returns, per line, the list of (character, fg, bg, bold)."""
    linhas = []
    frente = fundo = None
    negrito = False
...
def svg(linhas, titulo):
    ...
    # backgrounds first, grouping neighboring columns of the same color
    for y, linha in enumerate(linhas):
        x = 0
        while x < len(linha):
            f = linha[x][2]
    ...
    for y, linha in enumerate(linhas):
        ...
        while x < len(linha):
            ...
            # Leading space is dropped along with its coordinate: the browser
            # discards whitespace at the start of text and, if the positions
            # stayed, the rest of the line would shift left.
